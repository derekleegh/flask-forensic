...
class DDRecovery:
    def __init__(self, filename, required_info):
    ...
    def recover_dd(self):
        ...
        print(f"Performing dd extract on {self.filename}")
        if os.path.exists(self.output_dir):
            shutil.rmtree(self.output_dir)
        result = subprocess.run(["foremost", self.filename,], capture_output=True, text=True)
...
if __name__ == "__main__":
    ...
    processor = DDRecovery(args.filename, args.requiredInfo)
    processor.run()
